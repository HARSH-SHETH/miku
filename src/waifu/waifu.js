...
module.exports = async function sendWaifu(msg, client){
  ...
  try{
    ...
    let base64encodedImage = await Buffer.from(img.data, 'binary').toString('base64');
    let mimeType = url.slice(url.lastIndexOf('.')); 
    msg.reply(new MessageMedia('image/png', base64encodedImage, null));
  }catch(err){
    console.error(err);
  }
}
